...
Note that you can view the [github repo that generates this file and houses other
scripts here](https://github.com/jeffreyiacono/pml/tree/master).
...
that contains the new principle components calculated in the prior step. I also
used the "cv" (cross-validation) method to instruct train to perform
...
The model was trained over 10 iterations, each time being cross validated. The
final model produced an **out of sample error estimate of 2.19%**.

To cross-validate the final model, I calculated the sub-tests's principle
components and predicted against this data set, using the model from the
prior step:
...
Overall, **the accuracy was 0.97**, which is inline with our expectations for
out of sample error.
